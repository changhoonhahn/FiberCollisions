      ...
      parameter(Nsel=201,Nmax=2*10**8,Ngrid=1920,Nbin=151,pi=3.141592654)
...
c      complex dcg(Ngrid,Ngrid,Ngrid),dcr(Ngrid,Ngrid,Ngrid)
 ...
 13      continue
         ...
         gI33=0.d0
         do i=1,Ng
            nb=nbg(i) ! observed nbar(z)                     
            weight=dble(wg(i))/(1.d0+nbg(i)*P0/cmp(i)) ! no comp variation
            gI10=gI10+1.d0
            gI12=gI12+dble(weight**2)
            gI22=gI22+dble(nb*weight**2)
            gI13=gI13+dble(weight**3)
            gI23=gI23+dble(nb*weight**3 )
            gI33=gI33+dble(nb**2 *weight**3)
         enddo

         allocate(dcg(Ngrid,Ngrid,Ngrid))
         call assign(Ngal,rg,rm,Lm,dcg,P0,nbg,cmp,ig,wg)
 ...
 15      continue
         ...
         I33=0.d0
         do i=1,Nr
            nb=nbr(i)   ! observed nbar(z)  
            weight=dble(wr(i))/(1.d0+nbr(i)*P0/cmp(i))  ! no comp variations
            I10=I10+1.d0
...
c      parameter(Nmax=2*10**7)
     ...
     $           abs(rg(3,i)).lt.Rbox) then !put into box
      ...
      END
c%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
      subroutine assign(N,r,rm,L,dtl,P0,nbg,cmp,ig,w) !with FKP weighing
cc*******************************************************************
      real dtl(2*L,L,L),r(3,N),rm(2),we,ar,nb,nbg(N),w(N),cmp(N)
      integer ig(N)
      external nbar
c
      do 1 iz=1,L
