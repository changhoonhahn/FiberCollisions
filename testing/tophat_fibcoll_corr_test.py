...
def delP_corr_firstorder(k, Pk, l, fs=1.0, rc=0.4, extrap_params=[[3345.0, -1.6], [400.0, -4.]]): 
    '''
    Fiber collision correction term to Powerspectrum multipole l. The correlated term.
    ...
    for i_lp, lp in enumerate(np.arange(0.0, lp_max+0.1, 2)):
        ...
        for i_k, k_i in enumerate(k): 
            ...
            if i_k % 3 == 0: 
                start_time = time.time()
                integrand = lambda q_var: delPq_firstorder_integrand(
                        q_var, k_i, l, int(lp), Pk_interp, Pk_kplus_extrap, rc=rc, k_min=k[0], k_max=k[-1]
                        )
                delP[i_k] = quad_int(integrand, 0., np.inf, epsrel=0.01)[0]
                print "delP(k_i) first order calculation takes ", time.time() - start_time 
...
def delPq_firstorder_integrand(q, k, l, lp, f_interp, f_extrap, rc=0.4, k_min=0.002, k_max=0.3): 
    '''
    Integrand for del P_l^corr correction 
    ...
    '''
    if (q > k_min) and (q < k_max): 
        integrand = q * f_interp(q) * f_l_lp_firstorder(q*rc, k*rc, l, lp)
    elif q <= k_min: 
        integrand = 0.0
    elif q >= k_max: 
        integrand = 0.0
...
def f_l_lp_firstorder(x, y, l, lp, error=False): 
    '''
    f_l,l' term in the FC correction equation 
    ...
    fllp = lambda mu : f_l_lp_firstorder_integrand(mu, x, y, l, lp)
...
def f_l_lp_firstorder_integrand(mu, x, y, l, lp): 
    '''
    Integrand of f_l_lp integration 
    '''
    Leg_l = legendre(l)     # Legendre polynomial of order l 
    ...
    return Leg_l(mu) * Leg_lp(y * mu/x) * W_2d(theta)
...
if __name__=='__main__': 
    ...
    Pk = [P0k_sum/np.float(n_mocks), P2k_sum/np.float(n_mocks)]#, P4k]
    #for l1 in [2]:
    #    for l2 in [0,2,4]:
    #        for k_i in [1e-8, 1e-3, 0.002, 0.3, 1.0, 2.0, 10.0, 100.0]: 
    #            print l1, l2, f_l_lp(k_i*0.4, 0.3*0.4, l1, l2)
    ...
    #pickle.dump( delp0, open('delP0k_corr_downsampled.p', 'wb'))
    delp2 = delP_corr(k, Pk, 2, fs=1.0, rc=0.4, extrap_params=[[3345.0, -1.6], [400.0, -4.]])
    pickle.dump( delp2, open('delP2k_corr_downsampled.p', 'wb'))
    #delp0 = delP_corr_firstorder(k, Pk, 0, fs=1.0, rc=0.4, extrap_params=[[3345.0, -1.6], [400.0, -4.]])
    #pickle.dump( delp0, open('delP0k_corr_noextrap_firstorder.p', 'wb'))
    #delp2 = delP_corr_firstorder(k, Pk, 2, fs=1.0, rc=0.4, extrap_params=[[3345.0, -1.6], [400.0, -4.]])
    #pickle.dump( delp2, open('delP2k_corr_noextrap_firstorder.p', 'wb'))
