      ...
      integer ix,iy,iz,Nbins,nyq,iky,ikz,imk,i,Ibin,Ng,Nr
...
c      complex dcg(Ngrid/2+1,Ngrid,Ngrid),dcr(Ngrid/2+1,Ngrid,Ngrid)
      ...
      real*8, allocatable :: avgPg2(:),avgPr2(:),avgPg4(:),avgPr4(:)
      ...
      real cot1,coga,Le2,Le4,pk,rk
      complex ct
      
      call getarg(1,lssfname) !Data Fourier File
      call getarg(2,randfname) !Random Fourier File
      call getarg(3,powerfname) !power spectrum file
      call getarg(4,sscale) !Lbox (Mpc/h)
      read(sscale,*) akfun
      ...
      allocate(avgk(Nbins),avgPg(Nbins),avgPr(Nbins))
      allocate(co(Nbins),dk(Nbins))
      allocate(avgPg2(Nbins),avgPr2(Nbins))
      allocate(avgPg4(Nbins),avgPr4(Nbins))
