pro get_pth_fibcoll_dlos, n
; Obtain dLOS separation for fibercollided pairs in PTHalo Mock Catalogs
; spherematch is used to compare upweighted galaxies with unweighted galaxies
    mock_dir     = '/mount/riachuelo1/hahn/data/PTHalo/v11p0/'
    mock_file    = 'cmass_dr11_north_ir4'+strmid(strtrim(string(n+1000),1),1)+'.v11.0.wghtv.txt'
    
    readcol, mock_dir+mock_file, mock_ra, mock_dec, mock_redshift, ipoly, w_boss, w_cp, w_red, $
        redtrue, flag, m1, m2, veto
...
;Galaxies with up-weighted w_cp
    ...
    datadir = '/mount/riachuelo1/hahn/data/PTHalo/v11p0/'
    openw, lun, datadir+'DLOS_cmass_dr11_north_ir4'+strmid(strtrim(string(n+1000),1),1)+'.v11.0.wghtv.dat', /get_lun
    error_count=0
    disp_los = []
