#!/bin/bash
# Using Florian Beutler et al. 2013 to calculate I values
...
ifort -O3 -o exe/FFT_ldg_fkp_now_Iflorian_360grid.exe FFT_ldg_fkp_now_Iflorian_360grid.f -L/usr/local/fftw_intel_s/lib -lsfftw -lsfftw
ifort -O3 -o exe/FFT_ldg_fkp_w_Iflorian_360grid.exe FFT_ldg_fkp_w_Iflorian_360grid.f -L/usr/local/fftw_intel_s/lib -lsfftw -lsfftw
ifort -O3 -o exe/power_ldg_fkp_Iflorian_360grid_180bin.exe power_ldg_fkp_Iflorian_360grid_180bin.f

FFTrandname=$FFTdir$FFT$rand_file".Iflorian.grid"$grid".P0"$P0".box"$box  # same FFT for all correction method   
if [[ ! -a $FFTrandname ]]; then
    ./exe/FFT_ldg_fkp_now_Iflorian_360grid.exe $Rbox 1 $P0  $randdir$rand_file $FFTrandname
    echo $FFTrandname
fi
...
if [ $correction = "true" ]; then 
    ...
    for i in $(seq -f "%02g" 1 $n); do
        for letter in "a" "b" "c" "d"; do
            ...
            FFTname=$FFTdir$FFT$mock_prefix$i$letter$mock_suffix".Iflorian.grid"$grid".P0"$P0".box"$box
            ./exe/FFT_ldg_fkp_now_Iflorian_360grid.exe $Rbox 0 $P0 $mock_file $FFTname & 
            echo $FFTname
            jobcnt=(`jobs -p`)
            if [ ${#jobcnt[@]} -gt 3 ]; then
                wait
    ...
    for i in $(seq -f "%02g" 1 $n); do
        for letter in "a" "b" "c" "d"; do
            echo $i$letter
            FFTname=$FFTdir$FFT$mock_prefix$i$letter$mock_suffix".Iflorian.grid"$grid".P0"$P0".box"$box
            powername=$powerdir$power$mock_prefix$i$letter$mock_suffix".Iflorian.grid"$grid".P0"$P0".box"$box
            ./exe/power_ldg_fkp_Iflorian_360grid_180bin.exe $FFTname $FFTrandname $powername $sscale & 
            echo $powername
            jobcnt=(`jobs -p`)
            if [ ${#jobcnt[@]} -gt 3 ]; then
                wait
    ...
    if [ $correction = "delta" ]; then 
        ...
        for i in $(seq -f "%02g" 1 $n); do 
            for letter in "a" "b" "c" "d"; do 
                echo $i$letter
                fibcoll_file=$fibcoll_datadir$mock_prefix$i$letter$fibcoll_suffix
                FFTname=$FFTdir$FFT$mock_prefix$i$letter$fibcoll_suffix".delta.Iflorian.grid"$grid".P0"$P0".box"$box
                ./exe/FFT_ldg_fkp_w_Iflorian_360grid.exe $Rbox 0 $P0 $fibcoll_file $FFTname &
                echo $FFTname
                jobcnt=(`jobs -p`)
                if [ ${#jobcnt[@]} -gt 3 ]; then
                    wait
        ...
        for i in $(seq -f "%02g" 1 $n); do 
            for letter in "a" "b" "c" "d"; do 
                echo $i$letter
                FFTname=$FFTdir$FFT$mock_prefix$i$letter$fibcoll_suffix".delta.Iflorian.grid"$grid".P0"$P0".box"$box
                powername=$powerdir$power$mock_prefix$i$letter$fibcoll_suffix".delta.Iflorian.grid"$grid".P0"$P0".box"$box
                ./exe/power_ldg_fkp_Iflorian_360grid_180bin.exe $FFTname $FFTrandname $powername $sscale & 
                echo $powername
                jobcnt=(`jobs -p`)
                if [ ${#jobcnt[@]} -gt 3 ]; then
        ...
        for i in $(seq -f "%02g" 1 $n); do
            for letter in "a" "b" "c" "d"; do
                ...
                if [[ ! -a $peakcorr_file ]]; then
                     ./exe/peakcorr_ldg_fibcollmock.exe $sigma $nbar_file $mock_file $peakcorr_file &
                fi
                jobcnt=(`jobs -p`)
                if [ ${#jobcnt[@]} -gt 3 ]; then
                    wait
        ...
        for i in $(seq -f "%02g" 1 $n); do
            for letter in "a" "b" "c" "d"; do
                echo $i$letter
                peakcorr_file=$fibcoll_datadir$mock_prefix$i$letter$sigma_suffix
                FFTname=$FFTdir$FFT$mock_prefix$i$letter$sigma_suffix".Iflorian.grid"$grid".P0"$P0".box"$box
                ./exe/FFT_ldg_fkp_w_Iflorian_360grid.exe $Rbox 0 $P0 $peakcorr_file $FFTname &
                echo $FFTname
                jobcnt=(`jobs -p`)
                if [ ${#jobcnt[@]} -gt 3 ]; then
                    wait
        ...
        for i in $(seq -f "%02g" 1 $n); do
            for letter in "a" "b" "c" "d"; do
                echo $i$letter
                FFTname=$FFTdir$FFT$mock_prefix$i$letter$sigma_suffix".Iflorian.grid"$grid".P0"$P0".box"$box
                powername=$powerdir$power$mock_prefix$i$letter$sigma_suffix".Iflorian.grid"$grid".P0"$P0".box"$box
                ./exe/power_ldg_fkp_Iflorian_360grid_180bin.exe $FFTname $FFTrandname $powername $sscale &
                echo $powername
                jobcnt=(`jobs -p`)
                if [ ${#jobcnt[@]} -gt 3 ]; then
