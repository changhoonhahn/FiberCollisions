...
corr_suffix="_z0p342_fof_b0p2.zdist.fibcoll.dat"
mock_suffix="_z0p342_fof_b0p2.zdist.dat"
...
ifort -O4 -o FFT_lasdamasbox_aniso.exe FFT_lasdamasbox_aniso.f -L/usr/local/fftw_intel_s/lib -lsrfftw -lsfftw
...
for i in $(seq -f "%03g" 1 $n); do 
    ...
    mock_file=$mockdir$data_prefix$i$corr_suffix
    parm_file=$mockdir$data_prefix$i$parm_suffix
    FFT_file=$FFTdir$FFT$data_prefix$i$mock_suffix".grid"$grid".P0"$P0".box"$box
    power_file=$powerdir$power$data_prefix$i$mock_suffix".grid"$grid".P0"$P0".box"$box
#    ./export_lasdamas_ff.exe $data_file $mock_file $parm_file
    ./FFT_lasdamasbox_aniso.exe $Lx $Ly $Lz $parm_file $mock_file $FFT_file
