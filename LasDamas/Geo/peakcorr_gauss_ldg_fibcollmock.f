...
c galaxy file input: ra,dec,z,wcp
      ...
      read(sigmastr,*) sigma
      call getarg(2, peakfracstr)
      read(peakfracstr,*) peakfrac
      call getarg(3,nbarfile)
            ...
            ELSEIF (wcp.gt.1) THEN
                ...
                DO WHILE (wcp .gt. 1)
                    ...
                    ELSE
                        ...
                        ! randomly sample from 0.16 to 0.44 since nbar is constant
                        ran2=0.16+ran2*0.28
                        wcp=wcp-1 
                        wg(Ngal+1)=1.0
                        wsys=wsys+wg(Ngal+1)
...
c^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
      ...
      peakar=QQQ
      sigma=sigsig 
      peakpofr=EXP(-1.0*(peakar**2)/(sigma**2)) 
      RETURN
      END
c%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
