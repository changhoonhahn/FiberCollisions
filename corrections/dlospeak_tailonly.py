...
class DlospeakTailonlyCorr(Corrections): 
    ...
    def __init__(self, cat_corr, **kwargs): 
        ...
        * dLOS within peak is sampled +/- 3-sigmas
        ...
        super(DlospeakTailonlyCorr, self).__init__(cat_corr, **kwargs)
    ...
    def corrstr(self): 
        ...
        self.corr_str = ''.join([
            ...
            '.f_peakcorr', str(corr['f_peakcorr'])
            ])
    ...
    def build(self): 
        ...
        n_peakcorr = int(np.rint(corrdict['f_peakcorr'] * np.float(len(i_tailcorr))))
        if n_peakcorr == 0: 
            random_tail_pick = np.array([])
        elif n_peakcorr >= len(i_tailcorr): 
            random_tail_pick = i_tailcorr
        else:
            random_tail_pick = np.random.choice(i_tailcorr, n_peakcorr, replace=False)
    
        if n_peakcorr > 0:  

            fc_mock.wfc[random_tail_pick] += 1.0 
            for i_tail in fc_mock.upw_index[random_tail_pick]:
                fc_mock.wfc[i_tail] -= 1.0  
