...
class DlospeakPeakonlyCorr(Corrections): 
    ...
    def __init__(self, cat_corr, **kwargs): 
        ...
        * dLOS within peak is sampled +/- 3-sigmas
        ...
        super(DlospeakPeakonlyCorr, self).__init__(cat_corr, **kwargs)
    ...
    def build(self): 
        ...
        n_peakcorr = int(np.rint(corrdict['f_peakcorr'] * np.float(len(i_peakcorr))))
        if n_peakcorr == 0: 
            random_peak_pick = np.array([])
        elif n_peakcorr >= len(i_peakcorr): 
            random_peak_pick = i_peakcorr
        else:
            random_peak_pick = np.random.choice(i_peakcorr, n_peakcorr, replace=False)

        if n_peakcorr > 0:  
            fc_mock.wfc[random_peak_pick] += 1.0 
            for i_peak in fc_mock.upw_index[random_peak_pick]:
                fc_mock.wfc[i_peak] -= 1.0
