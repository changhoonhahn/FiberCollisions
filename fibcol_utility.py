...
def fortran_code(fft_power, **cat_corr): 
    ...
    if catalog['name'].lower() == 'lasdamasgeo': 
        ...
        if fft_power.lower() == 'fft':      # FFT --------------------------------------------------------
            ...
            if correction['name'].lower() in ('true', 'upweight', 'peaknbar'):  # FKP estimator
                if spec['grid'] == 360: 
                    ...
                    'shotnoise', 'vlospeakshot', 'floriansn', 'hectorsn'): 
        ...
        # quadrupole codes ------------------------------------------------
            ...
            code_dir = '/home/users/hahn/powercode/FiberCollision/' 
    ...
    # Tiling Mocks ------------------------------------------------------------------------------------------------
        ...
        # code directory 
        code_dir = '/home/users/hahn/powercode/FiberCollision/TilingMock/'
        ...
        if fft_power.lower() == 'fft':
            ...
            if correction['name'].lower() in ('peakshot', 'allpeakshot', 'shotnoise', 'floriansn', 'hectorsn', 'vlospeakshot'):
                if spec['grid'] == 360: 
    ...
    # QPM --------------------------------------------------------------------------------------------------------
        ...
        code_dir = '/home/users/hahn/powercode/FiberCollision/QPM/dr12d/'
        ...
        if fft_power.lower() == 'fft': 
            ...
            elif correction['name'].lower() in ('peakshot', 'shotnoise', 'floriansn', 'hectorsn', 'vlospeakshot'): 
                if spec['grid'] == 360: 
...
def compile_fortran_code(code): 
    ...
    # compile command
    if code == '/home/users/hahn/powercode/FiberCollision/FFT_FKP_BOSS_cic_il4_v3.f': 
        compile_cmd = ' '.join(['ifort -fast -o', fort_exe, code, '-L/usr/local/fftw_intel_s/lib -lsrfftw -lsfftw -lm'])
    elif code == '/home/users/hahn/powercode/FiberCollision/power_FKP_SDSS_BOSS_v3.f': 
