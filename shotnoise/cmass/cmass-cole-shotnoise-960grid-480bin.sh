#!/bin/bash
# P(k) with Cole et al. 2005 shot noise 
...
ifort -O3 -o exe/FFT-cole-shotnoise-960grid.exe FFT-cole-shotnoise-960grid.f -L/usr/local/fftw_intel_s/lib -lsfftw -lsfftw
ifort -O3 -o exe/power-cole-shotnoise-960grid-480bin.exe power-cole-shotnoise-960grid-480bin.f
...
exe/FFT-cole-shotnoise-960grid.exe $Rbox 0 $P0 $cmass_file $FFT_file
echo $FFT_file
FFT_rand_file=$FFTdir$FFT$file_prefix$ns$file_flag$fft_ranext
if [[ ! -a $FFT_rand_file ]]; then 
    exe/FFT-cole-shotnoise-960grid.exe $Rbox 1 $P0 $random_file $FFT_rand_file
    echo $FFT_rand_file
fi

# Compute power spectrum for CMASS
power_file=$powerdir$power$file_prefix$ns$file_flag$output_flag$power_ext
exe/power-cole-shotnoise-960grid-480bin.exe $FFT_rand_file $FFT_file $power_file $sscale
echo $power_file
