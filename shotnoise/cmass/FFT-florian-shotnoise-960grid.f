! FFT code using FKP method for CMASS with Florian Shotnoise
! For CMASS data outputs N_gal,sys, and galaxy I12 
      implicit none  
      ...
      real, allocatable :: wg(:),wsys(:),wr(:),compg(:),compr(:)
      real selfun(Nsel),z(Nsel),sec(Nsel),zmin,zmax,az,ra,dec,rad,numden
      real wtot,wsystot,wnoz,wcp,n_bar,wfkp,comp
      real alpha,P0,nb,weight,ar,akf,Fr,Fi,Gr,Gi
      real*8 I10,I12,I22,I13,I23,I33,rwsys
      real*8 gI12
...
c      common /radint/Om0,OL0
     ...
     $     FFTW_ESTIMATE + FFTW_IN_PLACE)
      ...
      if (iflag.eq.0) then ! run on mock
         ...
         wtot=0.0
         do i=1,Nmax
            read(4,*,end=13)ra,dec,az,wsystot,wnoz,wcp,n_bar,comp
            ra=ra*(pi/180.)
            dec=dec*(pi/180.)
            rad=chi(az)
            wsys(i)=wsystot
            wg(i)=wsystot*(wnoz+wcp-1.0)
...
!         close(7)
         ...
         gI12=0.d0          ! compute galaxy I12 for florian shot noise calculations
         do i=1,Ng
            nb=nbg(i)
            wfkp=1./(1.+nb*P0)
            gI12=gI12+dble(wg(i)*wsys(i)*wfkp**2/compg(i))
         enddo 
       ...
       elseif (iflag.eq.1) then ! compute discretness integrals and FFT random mock
         ...
         rwsys=0.d0
         do i=1,Nmax
            read(4,*,end=15)ra,dec,az,n_bar,comp
            ra=ra*(pi/180.)
            dec=dec*(pi/180.)
            rad=chi(az)
 ...
 15      continue
         ...
         write(*,*)'Nr=',Nr,'Nran=',Nran,'Nr,sys=',rwsys
         ...
         do i=1,Nr
            ...
            weight=1./(1.+nb*P0) 
            I10=I10+1.d0
            I12=I12+dble(weight**2/compr(i))
            I22=I22+dble(nb*weight**2/compr(i))
            I13=I13+dble(weight**3/compr(i))
            I23=I23+dble(nb*weight**3/compr(i))
            I33=I33+dble(nb**2 *weight**3/compr(i))
         enddo
