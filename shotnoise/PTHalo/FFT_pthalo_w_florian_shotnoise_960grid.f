c FFT code for PTHalo Mock Catalogs using FKP weighting AND systematic weights 
c with Florian's shot noise. Ngrid=960
c code inputs: Lbox, galaxy/random flag, P0, nbar file, galaxy/random file, FFT output file
c nbar file inputs: zmid,zlow,zhigh,numden,garb1,garb2,garb3
c galaxy/random file inputs: ra,dec,z,ipoly,wboss,wcp,wred,redtru,flag,m1,m2,veto
c weighting scheme mimics real data: wboss(wcp+wrf-1) for galaxies with wb, wcp, wf > 0 and veto = 1
...
c      complex dcg(Ngrid,Ngrid,Ngrid),dcr(Ngrid,Ngrid,Ngrid)
 ...
 12   continue
      ...
      if (iflag.eq.0) then ! run on mock
         ...
         allocate(rg(3,Nmax),nbg(Nmax),ig(Nmax),wg(Nmax),wsys(Nmax))
     ...
     &      ,m2,veto
            ...
            nbg(i)=nbar(az)
            wsys(i)=float(wb)
            IF (wb.gt.0 .and. wcp.gt.0 .and. wred.gt.0 
     &      .and. veto.eq.1) THEN
                wg(i)=float(wb)*(float(wcp)+float(wred)-1.)
            ELSE
                wg(i)=0.0
            ENDIF
            Ngal=Ngal+1
 ...
 13      continue
         ...
         WRITE(*,*) 'Wsys=',wtot,'Ngal,sys=',wtot/float(Ngal)
         ...
         do i=1,Ng
            nb=nbg(i)
            wfkp=1./(1.+nb*P0)
            gI12=gI12+dble(wg(i)*wsys(i)*wfkp**2)
         enddo 
...
!         write(*,*)'Random Survey File'
         ...
         do i=1,Nmax
            ...
            dec=dec*(pi/180.)
            rad=chi(az)
            IF (rwb.gt.0. .and. rwcp.gt.0. .and. rwred.gt.0.
     &      .and. rveto.gt.0.) THEN
                wr(i)=rwb*(rwcp+rwred-1.)
            ELSE
                wr(i)=0.0
            ENDIF
