...
P0=20000; sscale=3600.0; Rbox=1800.0; box="3600"; grid="360"
...
ifort -O3 -o exe/FFT_pth_fkp_now_360grid.exe FFT_pth_fkp_now_360grid.f -L/usr/local/fftw_intel_s/lib -lsfftw -lsfftw
ifort -O3 -o exe/FFT_pth_fkp_wboss_360grid.exe FFT_pth_fkp_wboss_360grid.f -L/usr/local/fftw_intel_s/lib -lsfftw -lsfftw
ifort -O3 -o exe/FFT_pth_fkp_w_360grid.exe FFT_pth_fkp_w_360grid.f -L/usr/local/fftw_intel_s/lib -lsfftw -lsfftw
ifort -O3 -o exe/power_pth_fkp_360grid_180bin.exe power_pth_fkp_360grid_180bin.f
ifort -O3 -o exe/bisp_pthalo_360grid.exe bisp_pthalo_360grid.f -L/usr/local/fftw_intel_s/lib -lsrfftw -lsfftw
...
if [[ ! -a $FFTrandname ]]; then
    echo "Random FFT"
    ./exe/FFT_pth_fkp_now_360grid.exe $Rbox 1 $P0 $nbar_file $datadir$rand_file $FFTrandname
    echo $FFTrandname
fi
...
if [ $correction = "noweight" ]; then 
    ...
    for i in $(seq -f "%03g" $ni $nf); do
        ...
        ./exe/FFT_pth_fkp_now_360grid.exe $Rbox 0 $P0 $nbar_file $mock_file $FFTname & 
        echo $FFTname
        jobcnt=(`jobs -p`)
        if [ ${#jobcnt[@]} -gt 4 ]; then
            wait
    ...
    if [ $spectrum = "pk" ]; then 
        ...
        for i in $(seq -f "%03g" $ni $nf); do
            ...
            ./exe/power_pth_fkp_360grid_180bin.exe $FFTname $FFTrandname $powername $sscale & 
            echo $powername
            jobcnt=(`jobs -p`)
            if [ ${#jobcnt[@]} -gt 4 ]; then
                wait
    ...
    for i in $(seq -f "%03g" $ni $nf); do 
        ...
        ./exe/FFT_pth_fkp_wboss_360grid.exe $Rbox 0 $P0 $nbar_file $mock_file $FFTname & 
        echo $FFTname
        jobcnt=(`jobs -p`)
        if [ ${#jobcnt[@]} -gt 4 ]; then
            wait
    ...
    for i in $(seq -f "%03g" $ni $nf); do 
        ...
        ./exe/FFT_pth_fkp_w_360grid.exe $Rbox 0 $P0 $nbar_file $mock_file $FFTname & 
        echo $FFTname
        jobcnt=(`jobs -p`)
        if [ ${#jobcnt[@]} -gt 4 ]; then
            wait
    ...
    for i in $(seq -f "%03g" $ni $nf); do
        ...
        ./exe/FFT_pth_fkp_w_360grid.exe $Rbox 0 $P0 $nbar_file $peakcorr_file $FFTname &
        echo $FFTname
        jobcnt=(`jobs -p`)
        if [ ${#jobcnt[@]} -gt 4 ]; then
            wait
