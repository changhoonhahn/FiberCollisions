      ...
      parameter(Nsel=201,Nmax=2*10**8,Ngrid=1920,Nbin=151,pi=3.14159265)
...
!Arguments: Rbox, Mock/Random, P0, File, FFT file
      ...
      if (iflag.eq.0) then ! run on mock
         ...
         Ngsys=0.d0
         do i=1,Nmax
            read(4,*,end=13)ra,dec,az,nz,wfc
            ra=ra*(pi/180.)
            dec=dec*(pi/180.)
            rad=chi(az)
     ...
     &   real(gI33)
       ...
       elseif (iflag.eq.1) then ! compute discretness integrals and FFT random mock
         ...
         do i=1,Nmax
            ...
            rr(3,i)=rad*sin(dec)
            nbr(i)=nz            ! nbar_true no comp variation
            cmp(i)=1.0            ! save comp weights
