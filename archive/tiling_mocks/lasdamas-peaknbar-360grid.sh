...
ifort -O3 -o FFT-fkp-peaknbar-corr-lasdamas-mock-360grid.exe FFT-fkp-peaknbar-corr-lasdamas-mock-360grid.f -L/usr/local/fftw_intel_s/lib -lsfftw -lsfftw
...
if [[ ! -a $FFTrandname ]]; then
    ./FFT-fkp-peaknbar-corr-lasdamas-mock-360grid.exe $Rbox 1 $P0  $randdir$rand_file $FFTrandname
    echo $FFTrandname
fi
...
for i in $(seq -f "%02g" 1 $n); do 
    for letter in "a" "b" "c" "d"; do 
       ...
       ./FFT-fkp-peaknbar-corr-lasdamas-mock-360grid.exe $Rbox 0 $P0 $mock_file $FFTname
       echo $FFTname
