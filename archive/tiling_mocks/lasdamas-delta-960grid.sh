...
echo "n="; read n 
ifort -O3 -o FFT-fkp-delta-corr-lasdamas-mock-960grid.exe FFT-fkp-delta-corr-lasdamas-mock-960grid.f -L/usr/local/fftw_intel_s/lib -lsfftw -lsfftw
...
if [[ ! -a $FFTrandname ]]; then
    ./FFT-fkp-delta-corr-lasdamas-mock-960grid.exe $Rbox 1 $P0  $randdir$rand_file $FFTrandname
    echo $FFTrandname
fi
...
for i in $(seq 1 $n); do 
    for letter in "a" "b" "c" "d"; do 
        ...
        ./FFT-fkp-delta-corr-lasdamas-mock-960grid.exe $Rbox 0 $P0 $mock_file $FFTname
        echo $FFTname
