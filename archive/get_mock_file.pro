function get_mock_file, mock=mock, version=version, n_mock=n_mock
; Gets files once Mock Catalog, Version, and # of mock is specified
    if (strlowcase(mock) EQ 'pthalo') then begin 
        ...
        endelse 
        pthalo_file_name = 'cmass_dr11_north_ir4'+strmid(strtrim(string(n_mock+1000),1),1)+'.'+version_corr+'.wghtv.txt'

        mock_file_name = pthalo_mock_dir + pthalo_file_name 
    endif
    if (strlowcase(mock) EQ 'lasdamas') then begin
        if (version EQ 'geo') then begin 
            lasdamas_dir = '/mount/chichipio2/rs123/MOCKS/LRGFull_zm_geo/gaussian/zspace/' 
            lasdamas_n_mock = strmid(n_mock, 0, stregex(n_mock, '(a|b|c|d)'))
            lasdamas_letter = strmid(n_mock, stregex(n_mock, '(a|b|c|d)')) 
            lasdamas_file = 'sdssmock_gamma_lrgFull_zm_oriana'+strmid(strtrim(string(lasdamas_n_mock+100),1),1)+$
    ...
    if (strlowcase(mock) EQ 'tilingmock') then begin
        ...
        tilingmock_file = 'cmass-boss5003sector-icoll012.dat'
        mock_file_name = tilingmock_dir+tilingmock_file 
    endif

    if (file_test(mock_file_name) NE 1) then STOP
