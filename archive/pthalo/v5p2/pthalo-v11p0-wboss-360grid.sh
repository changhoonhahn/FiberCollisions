...
ifort -O3 -o FFT-fkp-pthalo-v11p0-wboss-360grid.exe FFT-fkp-pthalo-v11p0-wboss-360grid.f -L/usr/local/fftw_intel_s/lib -lsfftw -lsfftw
...
FFTrandname=$FFTdir$FFT"cmass_dr11_north_600_randoms_ir4_combined.v11.0.wboss.grid"$grid".P0"$P0".box"$box
...
if [[ ! -a $FFTrandname ]]; then 
    echo $FFTrandname 
    ./FFT-fkp-pthalo-v11p0-wboss-360grid.exe $Rbox 1 $P0 $datadir$nbarfname $datadir$randname $FFTrandname
fi
...
for i in $(seq -f "%03g" 1 $n); do 
    ...
    FFTname=$FFTdir$FFT$name0$i".v11.0.wboss.grid"$grid".P0"$P0".box"$box
    powername=$powerdir$power$name0$i".v11.0.600randoms.wboss.grid"$grid".P0"$P0".box"$box
    echo $FFTname 
    if [[ ! -a $FFTname ]]; then 
        ./FFT-fkp-pthalo-v11p0-wboss-360grid.exe $Rbox 0 $P0 $datadir$nbarfname $fname $FFTname
    fi
    echo $powername
