      ...
      REAL wbr,wcpr,wredr,vetor,ipoly,wb,wcp,wred,flag,veto
      REAL dlosrt,dlosprt
...
c      complex dcg(Ngrid,Ngrid,Ngrid),dcr(Ngrid,Ngrid,Ngrid)
 ...
 12   continue
      ...
      if (iflag.eq.0) then ! run on mock
         ...
         DO i=1,Nmax
            ...
            dec=dec*(pi/180.)
            rad=chi(az)
            IF (wcp.gt.0. .and. wb.gt.0. .and. wred.gt.0. .and. 
     *      veto.gt.0.) THEN 
                wg(i)=wb*(wcp+wred-1.0)
            ELSE 
                wg(i)=0.0
            ENDIF
 ...
 13      continue
       ...
       elseif (iflag.eq.1) then 
         ...
         wsysr=0.0
         do i=1,Nmax
            read(4,*,end=15)ra,dec,az,wbr,wcpr,wredr,vetor
            ra=ra*(pi/180.)
            dec=dec*(pi/180.)
            rad=chi(az)
            IF (wcpr.gt.0 .and. wbr.gt.0 .and. wredr.gt.0 .and. 
     &      vetor.gt.0) THEN 
                wr(i)=wbr*(wcpr+wredr-1.0)
            ELSE 
                wr(i)=0.0
            ENDIF
