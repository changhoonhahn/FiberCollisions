...
for i in nrange:
    for letter in ['a','b','c','d']:
        ...
        if (i==1 and letter=='a'):
            ...
            sum_true        += data_true[:,1]
            sum_delta       += data_delta[:,1]
avg_true        = sum_true/(float(n)*4.0)
avg_delta       = sum_delta/(float(n)*4.0)
...
ax11.loglog( k, avg_delta-avg_true, color='k', 
...
for i in nrange: 
    for letter in ['a','b','c','d']:
        file_peaknbar  = file_prefix+str(i+100)[1:3]+letter+file_midfix+'peaknbar.sigma5.3.fpeak1.0.fibcoll.dat.peaknbar'+file_suffix
        data_peaknbar   = np.loadtxt(dir+file_peaknbar)
        if (i==1 and letter=='a'):
            sum_peaknbar    = data_peaknbar[:,1]
        else:
            sum_peaknbar    += data_peaknbar[:,1]
avg_peaknbar    = sum_peaknbar/(float(n)*4.0)
...
ax12.plot( k, ratio_peaknbar_true,'r--', linewidth=3, 
    label=r"$\overline{P(k)}_{\rm{Peak}+\bar{n}(z);\sigma = 5.3,f_{\rm{peak}}=1.0}/\overline{P(k)}_{\rm{True}}$")
#ax12.set_ylim([0.85,1.1])
ax12.legend(loc='upper left',prop={'size':14})
fig1.savefig('/home/users/hahn/research/figures/boss/fiber_collision/lasdamas-fibcoll-peaknbarsigma5.3-delta-comparison-zlim-360grid.png',bbox_inches=0)
py.show()
