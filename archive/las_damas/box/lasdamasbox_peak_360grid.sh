#!/bin/bash
sigma=7.6
echo "BESTFIT PEAK SIGMA="$sigma
data_prefix="mock_gamma_lrg21p2_zmo_Oriana_1"
data_suffix="_z0p342_fof_b0p2.zdist.ff"
mock_suffix="_z0p342_fof_b0p2.zdist.fibcol.dat"
wght_suffix="_z0p342_fof_b0p2.zdist.fibcoll.peak.sigma."$sigma".dat"
...
ifort -O4 -o mock_lasdamasbox_fibcol_peak.exe mock_lasdamasbox_fibcol_peak.f -L/usr/local/fftw_intel_s/lib -lsrfftw -lsfftw
...
for i in $(seq -f "%03g" 1 $n); do 
    ...
    wght_file=$mockdir$data_prefix$i$wght_suffix
    ...
    ./mock_lasdamasbox_fibcol_peak.exe $sigma $mock_file $wght_file
    echo $wght_file
    ./FFT_lasdamasbox_delta_aniso.exe $Lx $Ly $Lz $parm_file $wght_file $FFT_file
    echo $FFT_file
