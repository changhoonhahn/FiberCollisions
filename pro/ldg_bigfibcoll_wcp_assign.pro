pro ldg_bigfibcoll_wcp_assign, n, letter 
...
; read LasDamas Geo mock
    ...
    upw_index = replicate(-1L, Ngal)     ; index of upweighted galaxy 
    print, min(gal1), max(gal1)
    ...
    print, min(upw_index), max(upw_index)

    print, float(n_elements(wcp[where(wcp GT 1.0)]))/float(n_elements(wcp))

    if (Ngal ne total(wcp)) then STOP 
    
    now = where(wcp EQ 0.0, ngal_now)  
    print, 'FiberCollided Fraction ', float(ngal_now)/float(n_elements(wcp))
    ; write mocks with fibercollision weights
    bigfc_file = '/mount/riachuelo1/hahn/data/LasDamas/Geo/'+$
        'sdssmock_gamma_lrgFull_zm_oriana'+$
        strmid(strtrim(string(n+100),1),1)+letter+'_no.rdcz.big_fibcoll.dat'

    openw, lun, bigfc_file, /get_lun
